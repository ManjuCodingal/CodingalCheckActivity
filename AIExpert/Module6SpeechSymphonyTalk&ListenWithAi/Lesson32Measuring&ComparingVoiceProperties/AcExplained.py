...
import time
import pyaudio
import numpy as np # for calculating duration
...
def plot_waveform(data, rate):
    samples = np.frombuffer(data, dtype=np.int16) # Convert raw bytes to numpy array of integers (audio samples). Each sample is a 16-bit integer (from -32768 to 32767) representing the amplitude of the sound wave at that moment in time.
